...
THIS_IS_NEW_VERSION = "0616_DEBUG_DATA_CHECK"
...
MIN_SCORE_SHORT = 0       # 디버그: 5% 전략 TOP 무조건 표시
MIN_SCORE_MID = 0         # 디버그: 30% 전략 TOP 무조건 표시
...
def get_market_tickers():
    ...
    # GitHub Actions에서는 오늘 날짜가 휴장일/데이터 미반영일이면 티커가 비는 경우가 있어
    # 최근 10일 중 정상 티커가 잡히는 날짜를 자동으로 찾음
    base = datetime.now()
    tickers = []
    ...
    for back in range(0, 10):
        ...
        try:
            ...
            if SCAN_KOSPI:
                for code in stock.get_market_ticker_list(day, market="KOSPI"):
                    name = stock.get_market_ticker_name(code)
                    temp.append((code, name))

            if SCAN_KOSDAQ:
                for code in stock.get_market_ticker_list(day, market="KOSDAQ"):
                    name = stock.get_market_ticker_name(code)
                    temp.append((code, name))

            if temp:
                print(f"티커 기준일: {day}, 원본 {len(temp)}개")
                tickers = temp
                break

        except Exception as e:
            print("티커 조회 실패:", day, e)

    # 그래도 실패하면 대표 종목으로 최소 테스트
    if not tickers:
        print("시장 티커 조회 실패. 대표 종목으로 테스트합니다.")
        tickers = [
            ("005930", "삼성전자"),
            ("000660", "SK하이닉스"),
            ("035420", "NAVER"),
    ...
    for code, name in tickers:
        ...
        if any(w in upper_name for w in exclude_words):
            continue
        filtered.append((code, name))

    print(f"최종 검색 대상: {len(filtered)}개")
    return filtered
...
def main():
    ...
    for i, (code, name) in enumerate(tickers, 1):
        try:
            checked_count += 1
            df_test = get_ohlcv(code)
            if df_test is not None:
                data_ok_count += 1
                metrics_test = build_signal_metrics(df_test)
                if metrics_test is not None:
                    metrics_ok_count += 1

                    short_score, short_reasons = score_short_5(metrics_test)
                    mid_score, mid_reasons = score_mid_30(metrics_test)
                    ...
                    result = {
                        ...
                        "mid_score": mid_score,
                        "mid_reasons": mid_reasons,
                        "short_message": format_short_5(name, code, metrics_test, short_score, short_reasons),
                        "mid_message": format_mid_30(name, code, metrics_test, mid_score, mid_reasons),
                    }
