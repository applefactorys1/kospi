...
THIS_IS_NEW_VERSION = "0616_KOSPI200_CSV"
...
def load_tickers_from_file():
    ...
    returndef get_market_tickers():
    """
    GitHub Actions에서 KRX 전체 티커 조회가 막히는 문제를 피하기 위해
    kospi200_tickers.csv 파일만 읽습니다.
    """
    path = TICKER_FILE

    if not os.path.exists(path):
        raise RuntimeError(f"{path} 파일이 없습니다. GitHub 루트에 업로드하세요.")
